...
def main() -> int:
    ...
    ap.add_argument(
        "--train",
        default="01_data/processed/CMAPSS_full_norm/FD001/train_FD001_full_norm.csv",
        help="Train sensor CSV (engine_id,cycle,os*,s*). Used for mapping fit if fit-mapping-on=train.",
    )
    ap.add_argument(
        "--test",
        default="01_data/processed/CMAPSS_full_norm/FD001/test_FD001_full_norm.csv",
        help="Test sensor CSV (engine_id,cycle,os*,s*). Used to produce decision-support anomaly.",
    )
    ap.add_argument(
        "--pred",
        default="02_notebooks_exports/RUL/FD001/FD001_Ozcan_AllRaws/predictions_cycle_all_rows.csv",
        help="Optional: predictions CSV to validate key alignment and to create a joined artefact.",
    )
    ap.add_argument(
        "--metrics-json",
        default="02_notebooks_exports/RUL/FD001/FD001_Ozcan_AllRaws/metrics_cycle_all_rows.json",
        help="Optional: metrics JSON to enforce sensor set from pipeline feature_columns.",
    )
    ap.add_argument(
        "--out",
        default="02_notebooks_exports/Anomaly/jet-cube-turbofan-mvp/outputs/fd001_anomaly_scores.csv",
        help="Output anomaly CSV (engine_id,cycle,anomaly_score + optional columns).",
    )
    ap.add_argument(
        "--out-join",
        default="02_notebooks_exports/Anomaly/jet-cube-turbofan-mvp/outputs/fd001_test_with_preds_and_anomaly.csv",
        help="Optional joined CSV (preds + anomaly fields).",
    )
    ap.add_argument(
        "--out-mapping",
        default="02_notebooks_exports/Anomaly/jet-cube-turbofan-mvp/outputs/fd001_anomaly_mapping_params.json",
