...
def _apply_out_suffix(path: str, suffix: str) -> str:
    if not suffix:
        return path
    p = Path(path)
...
def _infer_dataset_kind(dataset_arg: str, pred_path: str) -> str:
    if dataset_arg in {"cmapss", "ncmapss"}:
        return dataset_arg

    low = pred_path.lower()
    if "ncmapss" in low or "n-cmapss" in low:
        return "ncmapss"
    if "cmapss" in low:
        return "cmapss"

    try:
        sample = pd.read_csv(pred_path, nrows=1, usecols=["dataset_id"])
        if not sample.empty:
            ds = str(sample.iloc[0]["dataset_id"]).upper()
            if ds.startswith("DS"):
                return "ncmapss"
            if ds.startswith("FD"):
                return "cmapss"
...
def main() -> int:
    ...
    ap.add_argument(
        "--policy-config",
        default=None,
        help="Optional YAML policy config for v2 policy-engine orchestration.",
    )
    ap.add_argument(
        "--policy-version",
        choices=["v1", "v2"],
        default=None,
        help="Policy execution mode. Default: v1 (or v2 when --policy-config is provided).",
    )
    ap.add_argument(
        "--out-suffix",
        default=None,
        help="Output suffix. Default: '' for v1, '_v2' for v2 policy-engine output.",
    )
    ap.add_argument(
        "--dataset",
        choices=["auto", "cmapss", "ncmapss"],
        default="auto",
        help="Dataset type for v2 adapter routing (auto infer by default).",
    )
    ap.add_argument(
        "--emit-v1",
        default=None,
    ...
    effective_policy_version = args.policy_version or ("v2" if args.policy_config else "v1")
    if effective_policy_version == "v2" and not args.policy_config:
        raise ValueError("--policy-config is required when --policy-version=v2.")

    out_suffix = args.out_suffix
    if out_suffix is None:
        out_suffix = "_v2" if effective_policy_version == "v2" else ""
    ...
    if effective_policy_version == "v2":
        if not args.anomaly:
            raise ValueError("v2 policy-engine requires --anomaly CSV input.")
        ...
        dataset_kind = _infer_dataset_kind(args.dataset, args.pred)
        v2_out_path = _apply_out_suffix(args.out, str(out_suffix))

        if dataset_kind == "ncmapss":
            from decision_support.adapters.ncmapss_adapter import run_ncmapss_adapter

            adapter_result = run_ncmapss_adapter(
                rul_csv=args.pred,
                anomaly_csv=args.anomaly,
            ...
            adapter_result = run_cmapss_adapter(
                rul_csv=args.pred,
                anomaly_csv=args.anomaly,
                config_path=args.policy_config,
                out_csv=v2_out_path,
        ...
        print(
            f"[v2] dataset={dataset_kind} out={adapter_result.output_path} "
            f"join_keys={','.join(adapter_result.join_keys)} rows={adapter_result.rows}"
        )
