...
RE_DECISION_SUPPORT = re.compile(r"^fd(\d{3})_decision_support\.csv$", re.IGNORECASE)
RE_ANOMALY = re.compile(r"^fd(\d{3})_anomaly_scores\.csv$", re.IGNORECASE)
RE_RUL = re.compile(r"^fd(\d{3})_rul_predictions\.csv$", re.IGNORECASE)
...
def main() -> None:
    ...
    st.title("AeroTrace — Decision Support Dashboard (FD001–FD004)")
    ...
    with st.sidebar:
        ...
        outdir = Path(
            st.text_input(
                ...
                help="Decision-support CSV klasörü (ör. 05_demo/decision_support_v2_outputs).",
            )
        ).expanduser()
