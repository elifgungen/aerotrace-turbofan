...
python scripts/build_fd001_preprocess.py \
  ...
  --out-train 01_data/processed/train_FD001_full_norm.csv \
  --out-test 01_data/processed/test_FD001_full_norm.csv \
  --report-json data/outputs/fd001_preprocess_report.json

python scripts/anomaly_baseline_deviation.py \
  --dataset FD001 \
  --train_csv 01_data/processed/train_FD001_full_norm.csv \
  --test_csv 01_data/processed/test_FD001_full_norm.csv \
  --config configs/anomaly_thresholds.json \
  --outdir data/outputs

python scripts/standardize_rul_predictions.py \
  --dataset FD001 --split test \
  --in_csv 02_notebooks_exports/RUL/FD001/FD001_Ozcan_AllRaws/predictions_cycle_all_rows.csv \
...
Beklenen çıktılar:
- `01_data/processed/train_FD001_full_norm.csv`
- `01_data/processed/test_FD001_full_norm.csv`
...
python scripts/build_fd002_preprocess.py \
  --raw-dir data/raw/CMAPSSData \
  --out-train 01_data/processed/train_FD002_regimeaware_sensor_zscore_k6_onehot.csv \
  --out-test 01_data/processed/test_FD002_regimeaware_sensor_zscore_k6_onehot.csv \
  --report-json data/outputs/fd002_preprocess_report.json

python scripts/build_fd002_anomaly.py \
  --train_csv 01_data/processed/train_FD002_regimeaware_sensor_zscore_k6_onehot.csv \
  --test_csv 01_data/processed/test_FD002_regimeaware_sensor_zscore_k6_onehot.csv \
  --config configs/anomaly_thresholds.json \
  --outdir data/outputs

python scripts/standardize_rul_predictions.py \
  --dataset FD002 --split test \
  --in_csv 02_notebooks_exports/RUL/FD002/FD002_Ozcan_All/fd002_test_predictions_FD002_LOCAL_REGIMEAWARE_SENSOR_ZSCORE_K6_ONEHOT.csv \
...
Beklenen çıktılar:
- `01_data/processed/train_FD002_regimeaware_sensor_zscore_k6_onehot.csv`
- `01_data/processed/test_FD002_regimeaware_sensor_zscore_k6_onehot.csv`
