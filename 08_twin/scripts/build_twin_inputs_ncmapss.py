...
Inputs:
- RUL predictions: 02_notebook_exports/RUL/N-CMAPSS/**/ncmapss_DSxx_rul_predictions*.csv
- Anomaly scores:   02_notebook_exports/Anomaly/N-CMAPSS/OUTPUTS/ncmapss_DSxx_anomaly_scores.csv

Output per dataset:
- 08_twin/inputs/ncmapss_DSxx_twin_feed.csv

Audit output:
- 08_twin/inputs/ncmapss_twin_input_audit.json
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--rul-root",
        default="02_notebook_exports/RUL/N-CMAPSS",
        help="Root folder for RUL prediction outputs.",
    )
    p.add_argument(
        "--anomaly-root",
        default="02_notebook_exports/Anomaly/N-CMAPSS/OUTPUTS",
        help="Root folder for anomaly outputs.",
    )
    p.add_argument(
        "--out-root",
        default="08_twin/inputs",
...
def resolve_anomaly_file(anomaly_root: Path, dataset: str) -> Path:
    p = anomaly_root / f"ncmapss_{dataset}_anomaly_scores.csv"
    if not p.exists():
        raise FileNotFoundError(f"{dataset}: anomaly file not found: {p}")
    return p
